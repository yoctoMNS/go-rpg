...
- **doc コメントだけで十分に伝わるようにする。** 公開（大文字始まり）の型・関数には
  `// Xxx does ...` の形式で1〜2行の doc コメントを書く。それ以外の細かい実装コメントは書かない
- **実装を逐一ナレーションしない。** 「何をしているか」はコードそのものが語る。コメントは
  コードだけでは伝わらないこと（なぜその設計にしたか）に限定し、無ければ書かない
- マジックナンバーは基本的に名前付き定数にする。理由がコードから読み取れない場合のみ
  コメントを添える
